...
| PUT    | `/api/v1/users/me/mascot` | Choose how the creature looks. Its own address, like the avatar: one tap, and folding it into the profile PUT would mean sending name, weight and height along |
...
| R-7 | `avatarUrl` accepts only an object name this server generated, never a URL                                                | See U-2: https was the wrong axis, because the risk is third-party origin and not transport. The photo is uploaded through its own endpoint, see [avatar.md](avatar.md) |
| R-8 | A profile save leaves `avatarUrl` alone when the body omits it                                                            | Same rule as `timeZone`. See U-5                                                                                                                                        |
...
| --- | -------- | ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| U-2 | Medium   | `UpdateProfileRequest` | **Reopened and closed properly on 2026-09-15.** The https-only rule was not enough: any external address is still a third party, so the beacon worked exactly as described below with an `https://` URL. Confirmed on a running server before the second fix: `https://evil.example.com/beacon.png` was accepted with 200, and now answers 400. The field takes only the 32-hex object name the storage adapter produces, which cannot express a scheme or a host. Original finding: `avatarUrl` was validated only by `@Size(max = 500)`. It is rendered as the `src` of an `<img>` **in the partner's browser** (`PairPage.tsx` → `Avatar.tsx`), so whoever set it chose an address the partner's browser would fetch: an ordinary tracking URL returns the partner's IP and user agent to its owner on every visit to the pair screen | Reachable by any user against their own partner. Not exploited, since nothing is deployed | `@Pattern` accepting only `https://` with no whitespace or quote characters. `ProfileUpdateIT.anAvatarThatIsNotAnHttpsUrlIsRejected` covers five payloads. Proved non-vacuous: without the annotation all five are accepted with 200 |

| U-5 | Medium | `UserProfileService` | `PUT` replaced `avatarUrl` with whatever arrived, so omitting it cleared the photo, while `timeZone` on the next line deliberately preserves on absence. No screen sends the field, so every profile save wiped it. Measured on a running server: set the avatar, save the form once, avatar gone | A photo disappeared the first time the person changed their goal. It had been documented as accepted rather than fixed, which stopped being defensible once photos could be uploaded | Same null-coalesce as `timeZone` (R-8). Verified on the same server: the name survives a save unchanged |
...
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| 2026-09-21 | `users.mascot` (V30) and `PUT /users/me/mascot`: the creature's face came from the ROLE in the pair (`who === 'partner' ? 'f' : 'm'`), so whoever was "you" always got the same one. The first real user picked female at sign-up, saw the other face and asked how to change it; there was no way. Named after the drawing (SPROUT, BLOSSOM) rather than after a gender, and null means "never chose", so nobody's creature changes on its own | `fix/landing-copy`                  |
| 2026-09-15 | U-2 closed properly: `avatarUrl` no longer accepts any external URL, so the tracking beacon is gone. U-5 fixed rather than accepted: a profile save no longer clears the photo. Both confirmed against a running server                                                                                                                                                                                                                         | `fix/screen-sweep`                  |
| 2026-09-06 | U-2 fixed, first HTTP-level tests for the profile, document created                                                                                                                                                                                                                                                                                                                                                                             | #32                                 |
| 2026-09-08 | the edit form on react-hook-form + zod, mirroring `UpdateProfileRequest` field by field; each missing or out-of-range field gets its own message next to it, replacing one banner for "sex and activity level". The weight card uses the shared `WeightForm`, which fixed a save with no `catch`. First 6 component tests                                                                                                                       | `refactor/frontend-forms-and-tests` |
