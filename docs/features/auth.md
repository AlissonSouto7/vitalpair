...
| ------ | ---------------------- | -------------------------------------------------------- | ------------------- |
| POST   | `/register`            | Start a registration; answers 202 either way, no session | 5/min per IP ¹      |
...
| R-6  | A failed activation e-mail does not fail registration                                                                                               | The account already exists. A 500 would send the person back to register and hit "e-mail already taken"                                                 |
| R-7  | `register`, `forgot-password` and `resend-verification` answer identically whether or not the account exists                                        | Otherwise the endpoint is an account enumeration oracle                                                                                                 |
...
| R-15 | Registration issues no session: the account is created unverified and the link in the e-mail activates it                                           | A session in the answer would tell a new address from one that already has an account, which is the oracle R-7 exists to close                          |
| R-16 | Signing in with a password requires a confirmed address; Google sign-in does not                                                                    | Without it the activation link is optional, and registering somebody else's address would still get you in. Google already proves the address is theirs |
...
| A-11 | Medium        | `AuthService#register`  | Registration answered 422 "e-mail already registered" for a known address and 201 with a session for a new one                                                                                                              | Anyone with a list of addresses could learn which belong to users, which is the input to targeted phishing and to password attacks that only try accounts known to exist. Proved against staging: 422 for a known address, 201 for an unknown one | Both branches answer 202 with the same body and no session. A new address receives an activation link; an existing one receives a notice that somebody tried to register with it. Signing in with a password now requires a confirmed address, so registering somebody else's does not get you in. `RegisterEnumerationIT` compares the two answers field by field |
...
| No account enumeration on any public flow, registration included | `RegisterEnumerationIT` (5 cases) and `AuthFlowIT.forgotPasswordDoesNotRevealWhetherAnAccountExists` and the identical message on login                    | 2026-09-06 |
