...
| 2026-09-11 | Deploys stopped being a person on SSH: `ci.yml` became callable and `cd.yml` runs it, builds both images natively on arm64 and tags them with the commit, then deploys. A merge to `main` reaches staging on its own; a `v*` tag reaches production only after someone approves it. The server pulls with the run's own token rather than holding a credential, and `docs/runbooks/deploy.md` says what to do when any of it fails | `ci/deploy-pipeline`   |
