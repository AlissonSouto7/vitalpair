...
| AC-2 | Low      | `LogActivityRequest`                  | `loggedAt` has no `@PastOrPresent`, so an activity can be dated arbitrarily far into the past or future                                                                                                                                                                                                    | A future-dated activity lands in a day that has not happened; a backdated one does not score, because the streak gate refuses anything not after the last recorded day | Bounding it is right, but it interacts with the timezone question below, so both are worth solving together                 |
