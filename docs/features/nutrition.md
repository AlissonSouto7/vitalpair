...
| R-11 | An imported meal whose calories **and** every macro are zero is refused with 422                                                               | That is the shape "no nutrition data" takes, not the shape a food takes. Deliberately narrow: a hand-entered 0 kcal meal still saves, because black coffee and water really are zero, and a blanket "calories > 0" would make the diary wrong in the other direction |
| R-12 | Deleting a meal publishes `MealDeletedEvent`                                                                                                   | The feed keeps its own copy, so removing the diary row is half the deletion. See [feed.md](feed.md) R-9                                                                                                                                                              |
...
| N-8 | Medium | `NutritionService`, `MealEditor.tsx`, `SearchTab.tsx` | Open Food Facts returns entries with no nutrition information. The search screen marked them "sem info nutricional" and still gave them the same "+" as a complete item; the editor opened at 0 kcal with saving enabled, because an empty field read as zero. Confirmed on a running server: `caloriesKcal: 0` answered 201 and the meal reached the diary and the feed | A meal in the diary asserting a food has no calories, which is not what "no information" means. It also counted towards missions and the season | An item with no data gets a different button, the editor requires the calories field to be filled, and the server refuses the data-free import (R-11). Verified on a running server across all three cases: import with no data 201 -> 422, hand-entered zero stays 201, import with real data stays 201 |
...
| `NutritionPage.tsx` was 942 lines and is now 368            | It was decomposed into `DayList`, `SearchTab`, `FavoritesTab`, `PhotoTab`, `MealEditor` and `parts`, each with its own file                                                                                                                                                        | Done                                                                                                                   |
| The meal selector at the top of the screen is now redundant | The editor asks "which meal?" as its first step, so the screen asks the same thing twice: once before the person has chosen a food, once after. The top one still sets the default the editor opens with, so removing it is not a deletion, it needs the default to move somewhere | Next change to this screen. Left in place deliberately when the step flow landed, to avoid changing two things at once |
| Portions are matched by food name                           | `portions.ts` recognises families with a regular expression over the name, which is coarse and gets Brazilian breakfast foods right by design rather than by data. The catalogue should say its own serving sizes; Open Food Facts does not always carry the field                 | When a food source with serving data is added                                                                          |

## History

| Date       | Change                                                                                                                                                                                                                                                                                                                                                                                                                                      | Pull request                    |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------- |
| 2026-09-21 | Logging a meal became two questions (which meal, how much) instead of five per-100g fields, after the first real user called the screen "extremely horrible". Portions replace grams as the main path, the manual numbers stay one tap away, the editor scrolls into view (it opened 1346px off screen), the screen no longer opens on the locked photo tab, and the duplicate save bar is gone. The editor had no test at all; it has nine | `fix/landing-copy`              |
| 2026-09-15 | N-8: an Open Food Facts item with no nutrition data could be logged as a 0 kcal meal, from the screen and from the API. Deleting a meal now also removes its feed item                                                                                                                                                                                                                                                                      | `fix/screen-sweep`              |
