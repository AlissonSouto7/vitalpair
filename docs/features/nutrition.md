...
| --- | -------- | ------------------------- | ------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| N-1 | Medium   | `NutritionController`     | The search and photo endpoints had no rate limit, and the photo one calls a paid API | One account could issue unbounded paid calls                                                                                                   | `RateLimitFilter`: photo capped at 20 an hour per user. Search is unlimited, since it costs nothing and Open Food Facts is free |
...
| N-4 | Low           | `NutritionController#search`   | The query string is passed to Open Food Facts with no length bound                                       | A very long query is sent upstream and rejected there. No effect on this application beyond one wasted call       | Low value; the upstream rejects it and the call is free                                                                         |
