...
ssh vitalpair 'cd ~/vitalpair/deploy && ./scripts/smoke.sh https://staging.vitalpair.app'
```
...
```bash
ssh vitalpair 'docker ps --format "{{.Names}}\t{{.Status}}"'
```
...
```bash
ssh vitalpair 'docker logs --tail 80 vitalpair-staging-backend-1'
ssh vitalpair 'docker logs --tail 40 vitalpair-edge-nginx-1'
```
...
```bash
ssh vitalpair 'cd ~/vitalpair/deploy && cat env/.deployed-staging env/.previous-staging'
```
