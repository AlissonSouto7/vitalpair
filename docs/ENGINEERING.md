...
How the code is built and what "done" means. Read it before touching the code.
...
docs/design/                       mockups, colour law, voice and tone
docs/runbooks/                     operating procedures
deploy/                            compose files, edge proxy, scripts
...
- ADR 0001 stays in Portuguese, because an accepted ADR is not rewritten; its
  content is restated in `docs/ARCHITECTURE.md`.

## 3. Non-negotiables

- **Only the repository owner pushes.** No force push, no tag, no merge from
  anyone else.
...
- **Never invent a number.** Every count, percentage, timing or coverage figure
  comes from a command whose output was actually seen. If it was not measured,
...
- No narration and no emoji. Never write "Here we...", "As requested...",
  "Now let's...", or "This function simply...".
...
- Planning notes and scratch files never land in source files or commit
  messages.
...
With `MAIL_ENABLED=false` (the default in development) no e-mail leaves the
machine and the reset and verification links are not written anywhere. To
exercise those flows locally, set `MAIL_ENABLED=true` with
`SPRING_MAIL_HOST=localhost` and `SPRING_MAIL_PORT=1025`, and read the messages
...
- **New environment variables** are added to `.env.example` with a placeholder,
  never a real value.
- **Dependencies are pinned** to an explicit version, and GitHub Actions to a
  full commit SHA.
...
| [docs/adr/](adr/)                           | Architecture decision records, one per decision                                             |
| [docs/ARCHITECTURE.md](ARCHITECTURE.md)     | How the code is organised and why                                                           |
| [docs/features/](features/)                 | One living document per feature                                                             |
| [docs/design/](design/)                     | Mockups, the colour law, themes, voice and tone. Read before building or changing a screen. |
| [docs/runbooks/](runbooks/)                 | Deploy, rollback, backup and restore, incident response                                     |
