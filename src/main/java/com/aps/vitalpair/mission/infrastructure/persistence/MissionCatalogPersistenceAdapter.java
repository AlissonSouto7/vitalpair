...
public class MissionCatalogPersistenceAdapter implements MissionCatalogRepositoryPort {
    ...
    public MissionCatalogPersistenceAdapter(
            MissionJpaRepository repository, MissionPersistenceMapper mapper) {
    ...
    public List<Mission> findByKind(MissionKind kind) {
        return repository.findByKindOrderByCodeAsc(kind).stream().map(mapper::toDomain).toList();
    }
