...
public class PairService
    ...
    public InvitePreview getInvitePreview(String inviteCode) {
        Pair pair = pairRepository.findByInviteCode(inviteCode)
        ...
        String inviterName = inviterId == null
                ? null
                : userRepository.findById(inviterId).map(User::getName).map(this::firstName).orElse(null);

        boolean full = pair.getStatus() == PairStatus.ACTIVE
                || (pair.getUser1Id() != null && pair.getUser2Id() != null);
