    ...
    SecurityFilterChain filterChain(HttpSecurity http, JsonAuthenticationEntryPoint entryPoint) throws Exception {
        ...
        http.csrf(AbstractHttpConfigurer::disable)
                ...
                .authorizeHttpRequests(auth -> auth.requestMatchers(PUBLIC_PATHS)
                        ...
                        .permitAll()
                        // Profile photos, GET only. An <img> tag cannot send an Authorization
                        // header, and fetching every avatar with the token instead would cost
                        // browser caching on every screen that shows one. What stands in for
                        // authorization is the name: 128 bits of randomness, generated by the
                        // server, handed out only with a profile the caller may already read,
                        // and replaced whenever the photo changes. Read only on purpose: the
                        // upload and the delete below are still authenticated.
